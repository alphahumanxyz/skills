...
class _ChatAction {
  ...
  constructor(
    ...
    params: ChatActionInterface = { delay: 4, autoCancel: true }
  ) {
    this._client = client;
    this._chat = chat;
    this._action = action;
  ...
  async start() {
    this._request = new Api.messages.SetTyping({ peer: this._chat, action: this._action });
    this._running = true;
    this._update();
  }
  ...
  async stop() {
    ...
    if (this.autoCancel) {
      await this._client.invoke(
        new Api.messages.SetTyping({ peer: this._chat, action: new Api.SendMessageCancelAction() })
      );
    }
  }
...
export class _ParticipantsIter extends RequestIter {
  ...
  async _init({
    ...
    if (search && (filter || ty != helpers._EntityType.CHANNEL)) {
      ...
      this.filterEntity = (entity: Entity) => {
        return (
          utils.getDisplayName(entity).toLowerCase().includes(search!) ||
    ...
    if (ty == helpers._EntityType.CHANNEL) {
      ...
      const full = await this.client.invoke(
        new Api.messages.GetFullChat({ chatId: entity.chatId })
      );
  ...
  async _loadNextChunk(): Promise<boolean | undefined> {
    ...
    for (let i = this.requests.length - 1; i >= 0; i--) {
      ...
      if (
        participants instanceof Api.channels.ChannelParticipantsNotModified ||
...
class _AdminLogIter extends RequestIter {
  ...
  async _init(
    ...
    if (filterArgs && Object.values(filterArgs).find(element => element === true)) {
      eventsFilter = new Api.ChannelAdminLogEventsFilter({ ...filterArgs });
    }
...
export function iterParticipants(
  ...
  return new _ParticipantsIter(
    ...
    {},
    { entity: entity, filter: filter, offset: offset ?? 0, search: search, showTotal: showTotal }
  );
}
...
export async function kickParticipant(
  ...
  if (type === helpers._EntityType.CHAT) {
    ...
    if (user instanceof Api.InputPeerSelf) {
      request = new Api.channels.LeaveChannel({ channel: peer });
      resp = await client.invoke(request);
    } else {
