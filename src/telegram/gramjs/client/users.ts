...
import { _entityType, _EntityType, isArrayLike, returnBigInt, sleep } from '../Helpers';
import { MTProtoSender } from '../network';
import { RequestState } from '../network/RequestState';
import { Api } from '../tl';
import { parseID, getPeerId as peerUtils } from '../Utils';
  ...
  for (attempt = 0; attempt < client._requestRetries; attempt++) {
    ...
    try {
      ...
      if (
        ...
        const shouldRaise =
          e instanceof errors.PhoneMigrateError || e instanceof errors.NetworkMigrateError;
...
/** @hidden */
export async function getMe<T extends boolean, R = T extends true ? Api.InputPeerUser : Api.User>(
  client: TelegramClient,
  inputPeer: T
): Promise<R> {
  ...
  if (!client._selfInputPeer) {
    client._selfInputPeer = utils.getInputPeer(me, false) as Api.InputPeerUser;
  }
...
export async function getEntity(
  ...
  if (users.length) {
    users = await client.invoke(new Api.users.GetUsers({ id: users }));
  }
  if (chats.length) {
...
export async function _getEntityFromString(client: TelegramClient, string: string) {
  ...
  if (id != undefined) {
    ...
    if (isInvite) {
      const invite = await client.invoke(new Api.messages.CheckChatInvite({ hash: username }));
...
/** @hidden */
export async function getPeerId(client: TelegramClient, peer: EntityLike, addMark = true) {
  if (typeof peer == 'string') {
    const valid = parseID(peer);
    if (valid) {
...
export async function _getPeer(client: TelegramClient, peer: EntityLike) {
  ...
  const [i, cls] = utils.resolveId(returnBigInt(await client.getPeerId(peer)));
  return new cls({ userId: i, channelId: i, chatId: i });
}
...
export async function _getInputNotify(client: TelegramClient, notify: any) {
  ...
  return new Api.InputNotifyPeer({ peer: await client.getInputEntity(notify) });
}
