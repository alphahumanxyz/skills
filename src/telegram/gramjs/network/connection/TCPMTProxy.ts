...
export type MTProxyType = BasicProxyInterface & { secret: string; MTProxy: true };
export type SocksProxyType = BasicProxyInterface & { socksType: 4 | 5 };
...
class MTProxyIO {
  ...
  constructor(connection: TCPMTProxy) {
    ...
    this._packetClass = connection.PacketCodecClass as unknown as AbridgedPacketCodec;
  ...
  async initHeader() {
    ...
    const encryptKey = await sha256(Buffer.concat([Buffer.from(random.slice(8, 40)), secret]));
