...
export function getFileInfo(
  ...
  if (fileLocation.SUBCLASS_OF_ID == 354669666) {
    return { dcId: undefined, location: fileLocation, size: undefined };
  }
  let location;
...
const VALID_USERNAME_RE = new RegExp(
  '^([a-z]((?!__)[\\w\\d]){3,30}[a-z\\d]|gif|vid|' + 'pic|bing|wiki|imdb|bold|vote|like|coub)$',
  'i'
);
...
export function getInputPeer(entity: any, allowSelf = true, checkHash = true): Api.TypeInputPeer {
  ...
  if (entity instanceof Api.InputUser) {
    return new Api.InputPeerUser({ userId: entity.userId, accessHash: entity.accessHash });
  }
  if (entity instanceof Api.InputChannel) {
    return new Api.InputPeerChannel({ channelId: entity.channelId, accessHash: entity.accessHash });
  }
  ...
  if (entity instanceof Api.PeerChat) {
    return new Api.InputPeerChat({ chatId: entity.chatId });
  }
...
export function getInputChannel(entity: EntityLike) {
  ...
  if (entity instanceof Api.InputPeerChannel) {
    return new Api.InputChannel({ channelId: entity.channelId, accessHash: entity.accessHash });
  }
  _raiseCastFail(entity, 'InputChannel');
}
...
export function getInputUser(entity: EntityLike): Api.TypeInputUser {
  ...
  if (entity instanceof Api.InputPeerUser) {
    return new Api.InputUser({ userId: entity.userId, accessHash: entity.accessHash });
  }
...
export function getInputChatPhoto(photo: any): Api.TypeInputChatPhoto {
  ...
  if (photo.SUBCLASS_OF_ID === 0xd4eb2d74) {
    ...
    // crc32(b'InputFile'):
    return new Api.InputChatUploadedPhoto({ file: photo });
  }
  photo = getInputPhoto(photo);
  if (photo instanceof Api.InputPhoto) {
    return new Api.InputChatPhoto({ id: photo });
...
export function getAttributes(
  ...
  const name: string =
    typeof file == 'string' ? file : 'name' in file ? file.name || 'unnamed' : 'unnamed';
  ...
  attrObj.set(
    ...
    new Api.DocumentAttributeFilename({ fileName: name.split(/[\\/]/).pop() || '' })
  );
  if (isAudio(file)) {
...
export function getInputMedia(
  ...
  if (media instanceof Api.MessageMediaGame) {
    return new Api.InputMediaGame({
      id: new Api.InputGameID({ id: media.game.id, accessHash: media.game.accessHash }),
    });
  }
  ...
  if (media instanceof Api.MessageMediaDice) {
    return new Api.InputMediaDice({ emoticon: media.emoticon });
  }
  if (
  ...
  if (media instanceof Api.Poll) {
    return new Api.InputMediaPoll({ poll: media });
  }
  _raiseCastFail(media, 'InputMedia');
}
...
export function parseUsername(username: string): { username?: string; isInvite: boolean } {
  ...
  if (username.match(VALID_USERNAME_RE)) {
    ...
    return { username: undefined, isInvite: false };
  }
}
