...
export async function start(client: TelegramClient, authParams: UserAuthParams | BotAuthParams) {
  ...
  const apiCredentials = { apiId: client.apiId, apiHash: client.apiHash };
    ...
    while (1) {
      try {
        ...
        const { user } = (await client.invoke(
          new Api.auth.SignUp({ phoneNumber, phoneCodeHash, firstName, lastName })
...
export async function signInUserWithQrCode(
  ...
  const inputPromise = (async () => {
    while (1) {
      ...
      await Promise.race([authParams.qrCode!({ token, expires }), sleep(QR_CODE_TIMEOUT)]);
  ...
  try {
    ...
    if (
      ...
      const migratedResult = await client.invoke(
        new Api.auth.ImportLoginToken({ token: result2.token })
      );
...
export async function sendCode(
  ...
  try {
    ...
    const resendResult = await client.invoke(
      new Api.auth.ResendCode({ phoneNumber, phoneCodeHash: sendResult.phoneCodeHash })
    );
...
export async function signInBot(
  ...
  const { user } = (await client.invoke(
    new Api.auth.ImportBotAuthorization({ apiId, apiHash, botAuthToken })
  )) as Api.auth.Authorization;
  return user;
}
