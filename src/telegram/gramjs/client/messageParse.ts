...
export async function _replaceWithMention(
  ...
  try {
    entities[i] = new Api.InputMessageEntityMentionName({
      ...
      userId: (await client.getInputEntity(user)) as unknown as Api.TypeInputUser,
    });
    return true;
  } catch (e) {
    return false;
...
export function _getResponseMessage(
  ...
  for (const update of updates) {
    if (update instanceof Api.UpdateMessageID) {
      ...
      (update.message as unknown as Api.Message)._finishInit(client, entities, inputChat);
      if ('randomId' in request || isArrayLike(request)) {
        idToMessage.set(update.message.id, update.message as unknown as Api.Message);
      } else {
      ...
      'peer' in request &&
      getPeerId(request.peer) == getPeerId((update.message as unknown as Api.Message).peerId!)
    ) {
      ...
      if (request.media.poll.id == update.pollId) {
        const m = new Api.Message({
          ...
          media: new Api.MessageMediaPoll({ poll: update.poll!, results: update.results }),
          message: '',
          date: 0,
        });
