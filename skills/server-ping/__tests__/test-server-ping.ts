...
const _assertGreaterThan = (globalThis as any).assertGreaterThan as (a: number, b: number, msg?: string) => void;
...
_describe("start()", () => {
  ...
  _it("should not register cron without URL", () => {
    ...
    const mock = _getMockState();
    _assertEqual(
      Object.keys(mock.cronSchedules).length,
      0,
      "should not register cron without URL",
    );
  });
...
_describe("Tools", () => {
  ...
  _it("read-config should handle missing file", () => {
    ...
    _assert(
      ...
      "should return error when no config file",
    );
  });
});
