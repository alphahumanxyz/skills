...
export class MTProtoSender {
  ...
  public _updateCallback: (client: TelegramClient, update: UpdateConnectionState) => void;
  ...
  constructor(authKey: undefined | AuthKey, opts: DEFAULT_OPTIONS) {
    ...
    this._state = new MTProtoState(this.authKey, this._log, this._securityChecks);
    ...
    while (this._userConnected && !this.isReconnecting) {
      const appendAcks = () => {
        if (this._pendingAck.size) {
          const ack = new RequestState(new MsgsAck({ msgIds: Array(...this._pendingAck) }));
  ...
  _handleBadAuthKey(shouldSkipForMain: boolean = false) {
    ...
    if (this._isMainSender && this._updateCallback) {
      this._updateCallback(this._client, new UpdateConnectionState(UpdateConnectionState.broken));
