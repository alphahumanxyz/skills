...
export class CustomMessage extends SenderGetter {
  ...
  init({
    ...
    SenderGetter.initSenderClass(this, { senderId: senderId ? returnBigInt(senderId) : undefined });
  ...
  _finishInit(client: TelegramClient, entities: Map<string, Entity>, inputChat?: EntityLike) {
    this._client = client;
    ...
    if (this.action) {
      if (
        ...
        this._actionEntities = [
          entities.get(utils.getPeerId(new Api.PeerChannel({ channelId: this.action.inviterId }))),
        ];
      } else if (this.action instanceof Api.MessageActionChannelMigrateFrom) {
        this._actionEntities = [
          entities.get(utils.getPeerId(new Api.PeerChat({ chatId: this.action.chatId }))),
        ];
      }
    }
  }
  ...
  get document() {
    if (this.media instanceof Api.MessageMediaDocument) {
      if (this.media.document instanceof Api.Document) return this.media.document;
    } else {
      const web = this.webPreview;

      return web && web.document instanceof Api.Document ? web.document : undefined;
    }
    return undefined;
  }
  ...
  get webPreview() {
    if (this.media instanceof Api.MessageMediaWebPage) {
      if (this.media.webpage instanceof Api.WebPage) return this.media.webpage;
    }
  }
  ...
  get toId() {
    if (this._client && !this.out && this.isPrivate) {
      return new Api.PeerUser({ userId: _selfId(this._client)! });
    }
    return this.peerId;
  }
  ...
  async getReplyMessage(): Promise<Api.Message | undefined> {
    if (!this._replyMessage && this._client) {
      ...
      this._replyMessage = (
        await this._client.getMessages(this.isChannel ? await this.getInputChat() : undefined, {
      ...
      if (!this._replyMessage) {
        ...
        // directly by its ID.
        this._replyMessage = (
          await this._client.getMessages(this.isChannel ? this._inputChat : undefined, {
  ...
  async forwardTo(entity: EntityLike) {
    if (this._client) {
      ...
      const params = { messages: [this.id], fromPeer: (await this.getInputChat())! };
  ...
  async delete({ revoke } = { revoke: false }) {
    if (this._client) {
      return this._client.deleteMessages(await this.getInputChat(), [this.id], { revoke });
    }
  }
  ...
  async click({ i, j, text, filter, data, sharePhone, shareGeo, password }: ButtonClickParam) {
    if (!this.client) {
      return;
    }
    if (data) {
      ...
      const button = new Api.KeyboardButtonCallback({ text: '', data: data });
      return await new MessageButton(this.client, button, chat, undefined, this.id).click({
    ...
    if (this.poll) {
      ...
      return await this.client.invoke(
        new Api.messages.SendVote({ peer: this.inputChat, msgId: this.id, options: options })
      );
    }
    ...
    if (button) {
      return await button.click({ sharePhone: sharePhone, shareGeo: shareGeo, password: password });
    }
  }
    ...
    if (
      ...
      for (const row of this.replyMarkup.rows) {
        ...
        for (const button of row.buttons) {
          const btn = new MessageButton(this.client, button, chat, bot, this.id);
          tmp.push(btn);
