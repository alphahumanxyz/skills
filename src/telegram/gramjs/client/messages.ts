  ...
  async _loadNextChunk() {
    ...
    if (this._ty == _EntityType.CHANNEL) {
      try {
        r = await this.client.invoke(
          new Api.channels.GetMessages({ channel: this._entity, id: ids })
        );
      } catch (e: any) {
        if (e.errorMessage == 'MESSAGE_IDS_EMPTY') {
          r = new Api.messages.MessagesNotModified({ count: ids.length });
        } else {
          throw e;
        }
      }
    } else {
      r = await this.client.invoke(new Api.messages.GetMessages({ id: ids }));
      if (this._entity) {
    ...
    for (message of r.messages) {
      if (
        ...
        (fromId && utils.getPeerId(message.peerId) != utils.getPeerId(fromId))
      ) {
...
export async function sendMessage(
  ...
  if (message && message instanceof Api.Message) {
    ...
    if (formattingEntities == undefined) {
      [message, formattingEntities] = await _parseMessageText(client, message || '', parseMode);
    }
    if (!message) {
...
export async function forwardMessages(
  client: TelegramClient,
  entity: EntityLike,
  { messages, fromPeer, silent, schedule, noforwards, dropAuthor }: ForwardMessagesParams
) {
  ...
  for (let [chatId, chunk] of groupBy(messages, getKey) as Map<number, Api.Message[] | number[]>) {
    let chat;
    let numbers: number[] = [];
    ...
    const result = await client.invoke(request);
    sent.push(client._getResponseMessage(request, result, entity) as Api.Message);
  }
  return sent;
}
...
export async function editMessage(
  ...
  if (file) {
    const { fileHandle, media, image } = await _fileToMedia(client, { file, forceDocument });
    inputMedia = media;
  }
...
export async function deleteMessages(
  ...
  if (ty == _EntityType.CHANNEL) {
    for (const chunk of utils.chunks(ids)) {
      results.push(client.invoke(new Api.channels.DeleteMessages({ channel: entity, id: chunk })));
    }
  } else {
    for (const chunk of utils.chunks(ids)) {
      results.push(client.invoke(new Api.messages.DeleteMessages({ id: chunk, revoke: revoke })));
    }
  }
...
export async function _pin(
  ...
  if (message === 0) {
    return await client.invoke(new Api.messages.UnpinAllMessages({ peer: entity }));
  }
...
export async function markAsRead(
  ...
  if (maxIdIsUndefined) {
    if (message) {
      if (Array.isArray(message)) {
        maxId = Math.max(...message.map(v => utils.getMessageId(v) as number));
      } else {
  ...
  if (_entityType(entity) === _EntityType.CHANNEL) {
    return await client.invoke(new Api.channels.ReadHistory({ channel: entity, maxId }));
  } else {
...
export async function getCommentData(
  ...
  const result = await client.invoke(
    new Api.messages.GetDiscussionMessage({ peer: entity, msgId: utils.getMessageId(message) })
  );
  const relevantMessage = result.messages.reduce((p: Api.TypeMessage, c: Api.TypeMessage) =>
    p && p.id < c.id ? p : c
  );
  let chat;
  ...
  return { entity: utils.getInputPeer(chat), replyTo: relevantMessage.id };
}
