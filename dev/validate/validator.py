...
def validate_skill_py(skill_py_path: Path, dir_name: str) -> SkillResult:
  ...
  if not skill.name or not isinstance(skill.name, str):
    ...
    if skill.name != dir_name:
      result.warnings.append(f'Skill name "{skill.name}" does not match directory "{dir_name}"')
    ...
    if skill.hooks:
      for field_name in VALID_HOOKS:
        ...
        if val is not None and not callable(val):
          result.errors.append(f'Hook "{field_name}" must be callable, got {type(val).__name__}')
  ...
  if skill.has_setup:
    ...
    result.warnings.append("Setup hooks defined but has_setup is False — hooks will not be called")
    ...
    for opt in skill.options:
      ...
      if opt.default is not None:
        if opt.type == "boolean" and not isinstance(opt.default, bool):
          result.errors.append(f'Option "{opt.name}": default must be bool for boolean type')
        elif opt.type == "number" and not isinstance(opt.default, (int, float)):
          result.errors.append(f'Option "{opt.name}": default must be numeric for number type')
        elif opt.type == "text" and not isinstance(opt.default, str):
          result.errors.append(f'Option "{opt.name}": default must be string for text type')

      # tool_filter only on boolean options
      if opt.tool_filter is not None:
        if opt.type != "boolean":
          result.errors.append(f'Option "{opt.name}": tool_filter is only valid on boolean options')
        elif all_tool_names:
  ...
  if skill.entity_schema is not None:
    ...
    for et in es.entity_types:
      ...
      if skill.name and "." in et.type:
        ...
        result.warnings.append(
          f'Entity type "{et.type}" is not namespaced (convention: "{skill.name}.{et.type}")'
        )
