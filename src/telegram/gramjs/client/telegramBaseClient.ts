...
import { ConnectionTCPMTProxyAbridged, ProxyInterface } from '../network/connection/TCPMTProxy';
...
export abstract class TelegramBaseClient {
  ...
  /** @hidden */
  public _ALBUMS = new Map<string, [ReturnType<typeof setTimeout>, Api.TypeUpdate[]]>();
  /** @hidden */
  public _exportedSenderPromises = new Map<number, Promise<MTProtoSender>>();
  /** @hidden */
  private _exportedSenderReleaseTimeouts = new Map<number, ReturnType<typeof setTimeout>>();
  /** @hidden */
  ...
  constructor(
    ...
    if (this._proxy && 'MTProxy' in this._proxy) {
      ...
      initProxy = new Api.InputClientProxy({ address: this._proxy.ip, port: this._proxy.port });
    }
    ...
    while (true) {
      try {
        ...
        if (this.session.dcId !== dcId && !sender._authenticated) {
          ...
          const req = new Api.InvokeWithLayer({ layer: LAYER, query: this._initRequest });
  ...
  async _borrowExportedSender(
    ...
    if (!this._exportedSenderPromises.get(dcId) || shouldReconnect) {
      this._exportedSenderPromises.set(
        dcId,
        this._connectSender(existingSender || this._createExportedSender(dcId), dcId)
      );
    }
