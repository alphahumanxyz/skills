...
export class TelegramClient extends TelegramBaseClient {
   ...
   */
  signInUser(apiCredentials: authMethods.ApiCredentials, authParams: authMethods.UserAuthParams) {
  ...
  /**
     ...
     */
  buildReplyMarkup(
    buttons: Api.TypeReplyMarkup | undefined | ButtonLike | ButtonLike[] | ButtonLike[][],
  ...
  downloadProfilePhoto(
    entity: EntityLike,
    downloadProfilePhotoParams: downloadMethods.DownloadProfilePhotoParams = { isBig: false }
  ) {
   ...
   * ```
   */
  forwardMessages(entity: EntityLike, forwardMessagesParams: messageMethods.ForwardMessagesParams) {
   ...
   *  ```
   */
  deleteMessages(entity: EntityLike | undefined, messageIds: MessageIDLike[], { revoke = true }) {
    return messageMethods.deleteMessages(this, entity, messageIds, { revoke: revoke });
  }
  ...
  unpinMessage(
    ...
    return messageMethods.unpinMessage(this, entity, message, unpinMessageParams) as Promise<
      Api.messages.AffectedHistory | undefined
    >;
  }
  ...
  async connect() {
    ...
    await this._sender.send(new Api.InvokeWithLayer({ layer: LAYER, query: this._initRequest }));
    if (!this._loopStarted) {
      _updateLoop(this);
      this._loopStarted = true;
    }
  ...
  async getDC(
    ...
    if (!isNode || web) {
      switch (dcId) {
        case 1:
          return { id: 1, ipAddress: `pluto${downloadDC ? '-1' : ''}.web.telegram.org`, port: 443 };
        case 2:
          return { id: 2, ipAddress: `venus${downloadDC ? '-1' : ''}.web.telegram.org`, port: 443 };
        case 3:
          return {
            id: 3,
        ...
        case 4:
          return { id: 4, ipAddress: `vesta${downloadDC ? '-1' : ''}.web.telegram.org`, port: 443 };
        case 5:
          return { id: 5, ipAddress: `flora${downloadDC ? '-1' : ''}.web.telegram.org`, port: 443 };
        default:
    ...
    for (const DC of this._config.dcOptions) {
      if (DC.id === dcId && !!DC.ipv6 === this._useIPV6) {
        return { id: DC.id, ipAddress: DC.ipAddress, port: 443 };
      }
    }
    throw new Error(`Cannot find the DC with the ID of ${dcId}`);
  }
