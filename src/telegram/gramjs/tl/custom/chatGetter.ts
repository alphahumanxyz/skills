...
export class ChatGetter {
  ...
  get inputChat() {
    if (!this._inputChat && this._chatPeer && this._client) {
      try {
        this._inputChat = this._client._entityCache.get(utils.getPeerId(this._chatPeer));
      } catch (e) {}
    }
  ...
  get chatId() {
    return this._chatPeer ? returnBigInt(utils.getPeerId(this._chatPeer)) : undefined;
  }
