...
export class DirectDownloadIter extends RequestIter {
  ...
  async _init({
    ...
    this.request = new Api.upload.GetFile({ location: fileLocation, offset, limit: requestSize });
...
export function iterDownload(
  ...
  return new cls(
    ...
    limit,
    {},
    { fileLocation: file, dcId, offset, stride, chunkSize, requestSize, fileSize, msgData }
  );
}
...
function closeWriter(writer: BinaryWriter | { write: Function; close?: Function }) {
...
export async function downloadProfilePhoto(
  ...
  return client.downloadFile(loc, { outputFile: fileParams.outputFile, dcId });
}
