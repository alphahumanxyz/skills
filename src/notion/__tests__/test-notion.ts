...
const _assertEqual = (globalThis as any).assertEqual as (a: unknown, b: unknown, msg?: string) => void;
const _assertNotNull = (globalThis as any).assertNotNull as (v: unknown, msg?: string) => void;
const _assertContains = (globalThis as any).assertContains as (h: string, n: string, msg?: string) => void;
...
function configuredInit(additionalFetchResponses?: Record<string, { status: number; body: string }>): void {
  ...
  freshInit({
    config: { token: VALID_TOKEN, workspaceName: "Test Workspace" },
    fetchResponses,
  });
}
...
_describe("init()", () => {
  _it("should load config from store if available", () => {
    freshInit({
      config: { token: VALID_TOKEN, workspaceName: "My Workspace" },
    });
...
_describe("Page tools", () => {
  ...
  _it("notion-create-page should create page", () => {
    ...
    const result = _callTool("notion-create-page", {
  ...
  _it("notion-append-text should append content", () => {
    ...
    const result = _callTool("notion-append-text", {
      block_id: "page-123",
      text: "Hello world",
    });
    _assertEqual(result.success, true);
