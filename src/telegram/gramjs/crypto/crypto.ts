...
export async function pbkdf2Sync(password: any, salt: any, iterations: any, ...args: any) {
  const passwordKey = await crypto.subtle.importKey('raw', password, { name: 'PBKDF2' }, false, [
    'deriveBits',
  ]);
  return Buffer.from(
