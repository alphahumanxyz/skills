            ...
            try {
              ...
              await sender.send(
                isLarge
                  ...
                  : new Api.upload.SaveFilePart({ fileId, filePart: jMemo, bytes: bytesMemo })
              );
            } catch (err: any) {
...
export async function _fileToMedia(
  ...
  if (
    ...
    try {
      ...
      return { fileHandle: undefined, media: undefined, image: isImage };
    }
  }
  let media;
  let fileHandle;
  ...
  if (media != undefined) {
    ...
    media = new Api.InputMediaUploadedPhoto({ file: fileHandle });
  } else {
    // @ts-ignore
    ...
    if (!thumb) {
      ...
      uploadedThumb = await uploadFile(client, { file: uploadedThumb, workers: 1 });
    }
  ...
  return { fileHandle: fileHandle, media: media, image: asImage };
}
...
export async function _sendAlbum(
  ...
  for (const file of files) {
    ...
    if (
      ...
      const r = await client.invoke(new Api.messages.UploadMedia({ peer: entity, media }));
    ...
    albumFiles.push(
      new Api.InputSingleMedia({ media: media!, message: text, entities: msgEntities })
    );
  }
...
export async function sendFile(
  ...
  if (formattingEntities != undefined) {
    ...
    [caption, msgEntities] = await _parseMessageText(client, caption, parseMode);
  }
